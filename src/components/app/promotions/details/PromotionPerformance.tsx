...
import styles from "./PromotionPerformance.module.css";
...
export function PromotionPerformance({ promotion }: { promotion: Promotion }) {
  const { resolvedTheme } = useTheme();
  const [activeMetric, setActiveMetric] = useState<Metric>("uses");
  const [animatedValue, setAnimatedValue] = useState(0);

  const seriesData = useMemo(() => {
    const weights = [0.08, 0.11, 0.13, 0.16, 0.14, 0.18, 0.2];
    const uses = weights.map((weight) =>
      Math.round(promotion.usageCount * weight),
    );
    const saleFactor = promotion.type === "precio_fijo" ? promotion.value : 42;
    const sales = uses.map(
      (value) => Math.round(value * saleFactor * 100) / 100,
    );
    return { uses, sales };
  }, [promotion]);

  const metrics = useMemo(() => {
    const totalUses = promotion.usageCount;
    const saleFactor = promotion.type === "precio_fijo" ? promotion.value : 42;
    const totalSales = Math.round(totalUses * saleFactor * 100) / 100;
    const maxUses = Math.max(
      promotion.usageLimit ?? 100,
      Math.ceil((totalUses * 1.35) / 10) * 10 || 100,
    );
    const maxSales = Math.max(
      5000,
      Math.ceil((totalSales * 1.4) / 1000) * 1000 || 5000,
    );
    ...
    return {
      uses: {
        ...
        value: totalUses,
        max: maxUses,
        description: `Veces que se aplicó ${promotion.name} en pedidos durante el periodo seleccionado.`,
      },
      sales: {
        label: "Ventas",
        value: totalSales,
        max: maxSales,
        description: `Ingresos generados por pedidos donde se aplicó ${promotion.name} durante el periodo seleccionado.`,
      },
    };
  }, [promotion]);
  ...
  const chartOptions: ApexOptions = useMemo(
    () => ({
      ...
      tooltip: {
        ...
        custom: ({ series, seriesIndex, dataPointIndex }) => {
          ...
          const formatted =
            activeMetric === "sales"
              ? PEN_FORMATTER.format(value)
              : NUMBER_FORMATTER.format(value);

          return `
            <div style="min-width: 170px; padding: 12px; color: ${CHART_UI.tooltipText};">
              <div style="color: ${CHART_UI.tooltipTitle}; font-size: 13px; font-weight: 600; margin-bottom: 10px;">${day}</div>
              <div style="display: flex; align-items: center; gap: ${CHART_UI.tooltipGap}; font-size: 13px; line-height: 1;">
                ...
                <strong style="font-weight: 600;">${formatted}</strong>
              </div>
            </div>
          `;
        },
    ...
    [activeMetric, resolvedTheme, selectedMetric.label],
  );
    ...
    <section className="overflow-hidden rounded-xl border bg-card text-card-foreground">
      ...
      <div className="grid xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className={`${styles.chart} min-w-0 px-1 pb-1 pt-4`}>
          <ReactApexChart
            key={`${resolvedTheme}-${activeMetric}`}
            type="bar"
            height={350}
        ...
        <aside className="border-t p-5 xl:border-l xl:border-t-0">
          ...
          >
            {(Object.keys(metrics) as Metric[]).map((metricKey) => (
              <button
                key={metricKey}
                type="button"
                role="tab"
