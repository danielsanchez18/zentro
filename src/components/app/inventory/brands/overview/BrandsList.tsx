...
export function BrandsList({ slug, brands }: { slug: string; brands: InventoryBrand[] }) {
  ...
  return <section className="space-y-5 font-heading sm:rounded-xl sm:border sm:border-border sm:bg-card sm:p-5">
    ...
    {pageItems.length === 0 ? <div className="rounded-xl border border-dashed border-border"><EmptyState icon={SearchX} title="Sin marcas coincidentes" description="Prueba con otra búsqueda o limpia los filtros." /></div> : <>{view === "tabla" ? <BrandsTable brands={pageItems} {...actionProps} /> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{pageItems.map((brand) => <BrandCard key={brand.id} brand={brand} {...actionProps} />)}</div>}<Paginator totalResults={filtered.length} pageSize={PAGE_SIZE} currentPage={currentPage} onPageChange={setPage} /></>}
