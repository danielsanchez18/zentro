...
import { SelectedCustomerBadge } from "./SelectedCustomerBadge";
      ...
      <aside className="w-full rounded-xl border bg-card lg:sticky lg:top-4 lg:w-96">
        <div className="border-b p-4">
          ...
          <div className="mt-3 flex items-center gap-1">
            {(["mesa", "recojo", "delivery"] as PosServiceType[]).map(
              (type) => (
                <Button
                  key={type}
          ...
          <div className="mt-3 flex flex-col gap-2">
            ...
            <div className="grid grid-cols-[1fr_auto] gap-2">
              {selectedCustomer ? (
                <SelectedCustomerBadge customer={selectedCustomer} onRemove={onCustomerClear} />
              ) : (
                <Input placeholder={serviceType === "mesa" ? "Cliente o invitado (opcional)" : serviceType === "recojo" ? "Cliente para recojo *" : "Cliente para delivery *"} value={customer} onChange={(event) => onCustomer(event.target.value)} className="h-fit min-w-0 flex-1 px-4 py-2 text-sm font-heading" />
              )}
              <Button type="button" variant="outline" onClick={() => setCustomerSearchOpen(true)} className="h-full">Buscar</Button>
            </div>
            {serviceType === "recojo" && (
              <Input
                placeholder="Hora estimada de recojo"
                value={reference}
                onChange={(event) => onReference(event.target.value)}
                className="px-4 py-2 h-fit text-sm font-heading"
              />
            )}
            {serviceType === "delivery" && <><Input placeholder="Teléfono *" value={phone} onChange={(event) => onPhone(event.target.value)} className="h-fit px-4 py-2 text-sm font-heading" /><Input placeholder="Dirección de entrega *" value={address} onChange={(event) => onAddress(event.target.value)} className="h-fit px-4 py-2 text-sm font-heading" /></>}
          </div>
        </div>
